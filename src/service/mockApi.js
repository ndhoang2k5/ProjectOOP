...
/**
  * Lấy danh sách sách từ `GET /books/add/{name}
  * Hạn chế: Backend không hỗ trợ tìm kiếm, nên chức năng search sẽ không lọc.
  * @deprecated Sử dụng `searchBooks` thay thế.
  * @returns {Promise<Array>} Danh sách sách.
  * @throws {Error} Nếu có lỗi xảy ra trong quá trình gọi API.
 */
export const getBook = async () => {
  console.warn("Hàm getBook đã lỗi thời. Sử dụng searchBooks thay thế.");
  try {
    const response = await axios.get(`${API_BASE_URL}/books/add/`);
